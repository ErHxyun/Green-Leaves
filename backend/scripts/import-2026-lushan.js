...
const article=JSON.parse(fs.readFileSync(new URL('../content/2026-lushan.json',import.meta.url),'utf8'));
...
try {
 const result=await transaction(pool,async c=>{
  ...
  for(const block of article.blocks){
   let media_id=null;
   if(block.type==='image'){
    const disk=new URL('../../frontend/src/pictures/2026-lushan/'+block.file,import.meta.url);
    const bytes=fs.statSync(fileURLToPath(disk)).size;
    const url='/media/source/2026-lushan/'+block.file;
    media_id=(await c.query('INSERT INTO media_assets(id,url,metadata) VALUES($1,$2,$3) ON CONFLICT(url) DO UPDATE SET url=EXCLUDED.url RETURNING id',['lushan-2026-'+block.file.replace(/[^a-z0-9]/gi,'-'),url,{bytes,source:'user-provided',privacy_review_required:true}])).rows[0].id;
   }
