...
首页、我们的努力、关于我们和联系页面共享 Little Green Leaves AI Assistant，路由切换保留当前聊天，切换语言清空聊天。浮动按钮或 Escape 可收起窗口。管理后台不显示访客助手。
POST /api/v1/chat 接收 message、lang 和最多 8 条 history（{role: user/assistant, content}；兼容旧字符串历史）。助手可寒暄、一般问答、创意和志愿服务建议；机构事实仅依据已发布内容，生成自然摘要并附服务器验证的原文来源。没有依据或来源在生成期间变更时，不展示生成的机构答案。一般建议不代表机构承诺。它不能读取草稿、编辑发布、报名或收款。本应用不保存访客聊天；问题、最近对话与官网片段发送至现有 AI provider。生成式回答仍可能有误，应通过来源核对。

运行迁移后启用。PUBLIC_CHAT_ENABLED=false 可关闭；PUBLIC_CHAT_DAILY_LIMIT=50 默认限制全站每天 UTC 50 次模型尝试，数据库原子计数跨重启保留，失败调用也计数。另有每来源连接 IP 每分钟 6 次、每进程同时 2 次上限；不信任转发头，因此反向代理部署时访客可能共享分钟额度。额度是调用次数，不是美元硬预算；请另在 AI 提供商设置消费限制。最多 36 个候选原文片段、输出上限 1200 tokens，没有向量数据库费用。每次重新读取已发布内容并在返回前复核，无过期内容缓存。
